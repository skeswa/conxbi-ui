define(['extCore', 'extBase', 'extAll'], function (Ext) {
	...
	Ext.ux.FileTreePanel = Ext.extend(Ext.tree.TreePanel, {
	     ...
	     * @cfg {String} newdirText Default name for new directories (defaults to 'New Folder')
	     */
	    ,copyfileText:'Copied File'

	    /**
	     * @cfg {String} newdirUrl URL to use when creating new directory;
	     * this.url is used if not set (defaults to undefined)
	     */
	    ,copyfileUrl: 'http://localhost/newdir'
		/**
		 ...
		 */
	    ,renameUrl: 'http://localhost/newdir'
		/**
		...
		,initComponent:function() {
			...
			if(!this.loader) {
				this.loader = new Ext.tree.TreeLoader({
					 url:this.url+this.urlPathSuffix
					,baseParams:{cmd:'get',fileExt: this.fileExtensionFilter, exclDirs: this.exclDirs}
					,listeners:{
						beforeload:{scope:this, fn:function(loader, node) {
	                        ...
	                        loader.url = this.url+this.urlPathSuffix;
						}}
					}
				});
			}
			...
			this.addEvents(
	            ...
	            ,'newdir'
	            /**
	             * @event beforecopyfile
	             * Fires before new file is copied. Return false to cancel the event
	             * @param {Ext.ux.FileTreePanel} this
	             * @param {Ext.tree.AsyncTreeNode} node under which the new file is being copied
	             */
	            ,'beforecopyfile'
	            /**
	             * @event copyfile
	             * Fires after the new file has been successfully copied
	             * @param {Ext.ux.FileTreePanel} this
	             * @param {Ext.tree.AsyncTreeNode} new file that has been copied
	             */
	            ,'copyfile'
				/**
				 * @event newdirfailure
	...
	//				,menu:this.getContextMenu()
		...
		// }}}
	    ,setUrlPathSuffix: function(urlPathSuffix) {
					...
					switch(o.cmd) {
						case 'delete':
							if(true !== this.eventsSuspended) {
								this.fireEvent('delete', this, o.path);
							}
	                    ...
	                    case 'newdir':
	                        if(true !== this.eventsSuspended) {
	                            this.currentNode.id = o.id;
	                            this.currentNode.icon = o.icon;
	                            this.currentNode.attributes.canupload = o.canupload;
	                            this.currentNode.attributes.candownload = o.candownload;
	                            this.fireEvent('newdir', this, this.currentNode);
	                        }
	                        break;
	                    ...
	                    case 'copyfile':
	                        if(true !== this.eventsSuspended) {
	                            this.currentNode.id = o.id;
	                            this.currentNode.iconCls = o.iconCls;
	                            this.currentNode.attributes.canupload = o.canupload;
	                            this.currentNode.attributes.candownload = o.candownload;
	                            this.fireEvent('copyfile', this, this.currentNode);
	                        }
	                        break;

						case 'rename':
							this.updateCls(this.currentNode, o.oldname);
							if(true !== this.eventsSuspended) {
								this.fireEvent('rename', this, this.currentNode, options.params.newname, options.params.oldname);
							}
						break;
					}
					...
					switch(options.params.cmd) {
						...
						case 'rename':
							...
							if(options.oldParent) {
								options.oldParent.appendChild(this.currentNode);
							}
							// handle simple rename error
							else {
								this.currentNode.setText(options.oldName);
							}
							...
							if(true !== this.eventsSuspended) {
								this.fireEvent('renamefailure', this, this.currentNode, options.params.newname, options.params.oldname);
							}
						break;
	                    ...
	                    case 'copyfile':
	                        if(false !== this.eventsSuspended) {
	                            this.fireEvent('copyfilefailure', this, options.params.dir);
	                        }
	                        this.currentNode.parentNode.removeChild(this.currentNode);
	                        break;
						...
						case 'delete':
							if(true !== this.eventsSuspended) {
								this.fireEvent('deletefailure', this, this.currentNode);
							}
							this.currentNode.parentNode.reload.defer(1, this.currentNode.parentNode);
						break;
	...
	// {{{
	     ...
	     * @param {Ext.tree.AsyncTreeNode} node
	     */
	    ,copyFileToNode:function(node,fileTitle,iconCls,pathId,parentPathId) {

	        // fire beforenewdir event
	        if(true !== this.eventsSuspended && false === this.fireEvent('beforecopyfile', this, node)) {
	            return;
	        }
	        ...
	        appendNode.expand(false, false, function(n) {
	            // create new node
	            n.expand();
	            newNode = n.appendChild(new Ext.tree.AsyncTreeNode({text:fileTitle,iconCls:iconCls,leaf:true}));
	            newNode.attributes['sourcePathId'] = pathId;
		...
		,deleteNode:function(node) {
			...
			Ext.Msg.show({
				...
				,fn:function(response) {
					...
					var options = {
						 url:this.deleteUrl+this.urlPathSuffix
						,method:this.method
						,scope:this
	                    ,onSuccess:this.cmdCallback
	                    ,onFailure:this.cmdCallback
						,node:node
						,params:{
							 cmd:'delete'
	                        ,isleaf: node.leaf
		...
		,getContextMenu:function() {
			...
			if(!this.contextmenu) {
				...
				this.uploadPanel.setUrl(this.uploadUrl+this.urlPathSuffix);
			}
		...
		,onBeforeNodeDrop:function(e) {
			...
			var options = {
				 url:this.renameUrl+this.urlPathSuffix
				,method:this.method
				,scope:this
		...
		,onDestroy:function() {
			...
			// call parent
	        try {Ext.ux.FileTreePanel.superclass.onDestroy.call(this);} catch(error){}
		...
		,onNewDir:function(editor) {
			...
			var options = {
				 url:this.newdirUrl+this.urlPathSuffix
				,method:this.method
				,scope:this
	    ...
	    // {{{
	    /**
	     * copy new file directory handler
	     * @private
	     * runs after editing of new name is completed
	     * @param {Ext.Editor} editor
	     */
	    ,onCopyFile:function(editor) {
	        var path = this.getPath(editor.editNode);
	        var fileTitle = editor.editNode.text;
	        var pathId = editor.editNode.attributes['sourcePathId'];
	        var parentPathId =  editor.editNode.parentNode.id;
	        var options = {
	            url:this.copyfileUrl+this.urlPathSuffix
	            ,method:this.method
	            ,scope:this
	            ,node:editor.editNode
	            ,onSuccess:this.cmdCallback
	    ...
	    ,_enableRequestHeaders: function() {
	...
	/*        Ext.lib.Ajax.request = Ext.lib.Ajax.request.createInterceptor(function (method, uri, cb, data, options) {
	    ...
	    ,_executeRequest: function(options) {
	        var _headers =  {userid: ORYX.CONFIG["USER_ID"]};
	        options.scope.currentNode = options.node;
	        if (options.params.cmd === 'delete' || options.params.cmd === 'copyfile' || options.params.cmd === 'rename')
	            new Ajax.Request(options.url, {
	                ...
	                onSuccess: options.onSuccess.bind(this),
	                onFailure: options.onFailure.bind(this)
	            });
	        else
	            new Ajax.Request(options.url, {
	                method: options.method,
	                asynchronous: options.asynchronous,
	                postBody: options.params,
	                requestHeaders: _headers,
	                contentType:  "application/x-www-form-urlencoded",
	                onSuccess: options.onSuccess.bind(this),
	                onFailure: options.onFailure.bind(this)
